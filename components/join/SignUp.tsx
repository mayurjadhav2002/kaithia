...
const SignUp = () => {
	...
	const handleRequestOtp = async () => {
		if (!userDetails.phone_number || userDetails.phone_number.length < 10) {
			setResponse({msg: "Please enter a valid phone number"});
			return;
		}
		setLoading(true);
		try {
				...
				<div className='mt-6 '>
					{!otpSent ? (
						<div>
							...
							<p className='mt-2 text-xs text-gray-500'>
								Enter password if you{"'"}ve enabled 2fa else
								leave it empty
							</p>
...
const LoggedInComponent = () => {
	return (
		<p className='px-3 mt-6 text-center text-3xl text-green-500'>
